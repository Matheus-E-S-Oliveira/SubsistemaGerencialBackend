            ...
            using (var package = new ExcelPackage(new FileInfo(filePath)))
            {
                ...
                for (int row = 1; row <= 850; row++) // Supondo que a primeira linha é o cabeçalho
                {
                    ...
                    (
                        ...
                        dataInicopagamento: dataInicioPagamento.HasValue ? dataInicioPagamento.Value : (DateTime?)null,// DataInicioPagamento (coluna H, valor específico tratado como null)
                        assinadoPeloPortal: worksheet.Cells[row, 1].Text.ToLower() == "sim" ? true : worksheet.Cells[row, 1].Text.ToLower() == "não" ? false : (bool?)null, // AssinadoPeloPortal (coluna A, sim/true e não/false, aceita null)
                        dataFimTryal: dataFimTryal.HasValue ? dataFimTryal.Value : (DateTime?)null, // DataFimTryal (coluna G)
                    ...
                    var fazenda = new Fazenda(
                        ...
                        codigoFazenda: worksheet.Cells[row, 2].Text, // CodigoFazenda (coluna B, não nulo)
                        ...
                        nome: worksheet.Cells[row, 21].Text.ToUpper(), // Nome (coluna U)
